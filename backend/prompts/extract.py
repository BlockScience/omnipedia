    ...
    def update(self, other: "RequirementsDocument") -> "RequirementsDocument":
        ...
        for group_name, group in other.groups.items():
            if group_name not in self.groups:
                ...
                existing_requirements_ids = {
                    req.id for req in self.groups[group_name].requirements
                }
                for req in group.requirements:
                    if req.id not in existing_requirements_ids:
...
def extract_requirements_from_chunk(
    ...
    return [
        ell.user(f"""You are an expert at extracting requirements from style guides. Your task is to read a chunk of a style guide and extract all prescriptive requirements, organizing them into a structured JSON format.

Instructions:
- Read the provided chunk carefully.
- Identify all prescriptive statements (look for words like 'must', 'should', 'always', etc.).
- For each requirement, create a unique identifier in the format 'R{{id}}'.
- Provide a concise description, exact reference from the text, category, classification, where, and when.
- Organize the requirements into groups based on their categories.
- Do not include duplicates; ensure each requirement is unique.
...
JSON Format:
{RequirementsDocument.model_json_schema()}
